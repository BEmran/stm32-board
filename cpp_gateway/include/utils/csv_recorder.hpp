...
namespace utils
{
    ...
    private:
        ...
        std::unordered_map<std::string_view, std::size_t> header_index_;
        std::ofstream file_;
     ...
     *
     */
    class CSVStatesRecorder : public CSVRecorder
    {
    public:
        CSVStatesRecorder(std::string_view path, std::string_view prefix = "states")
            : CSVRecorder(std::filesystem::path(std::string(path)), prefix, headers::STATE) {}

        inline utils::CSVRecorder create_states_recorder(std::string_view path, std::string_view prefix = "states")
        {
            return utils::CSVRecorder(std::filesystem::path(std::string(path)), prefix, headers::STATE);
        }
        /**
         ...
         * @return true if successful, false otherwise
         */
        bool record_state(const core::Timestamps &ts, const core::States &state);
