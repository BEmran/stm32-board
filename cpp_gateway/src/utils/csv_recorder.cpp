...
namespace utils
{
    ...
    CSVRecorder::CSVRecorder(
        ...
        : csv_path_(build_path(recorder_dir, prefix)), header_(std::move(header)), header_index_(), buffer_(std::make_unique<char[]>(buffer_size)), buffer_size_(buffer_size)
    {
        if (header_.empty())
        {
        ...
        header_index_.reserve(header_.size());
        for (std::size_t i = 0; i < header_.size(); ++i)
        {
            header_index_.emplace(header_[i], i);
        }
    }
        ...
        std::string_view prefix) const
    {
        ...
        // Build filename
        const auto timestamp = timestamp_string();
        std::string filename;
        if (!prefix.empty())
        {
            filename = std::string(prefix) + "_" + timestamp + ".csv";
        }
        else
        {
    ...
    {
        std::vector<std::string_view> values(header_.size());
    ...
    bool CSVStatesRecorder::record_state(const core::Timestamps &ts, const core::States &state)
    {
        if (!is_open())
        {
            std::cerr << "Error: CSVRecorder is not open\n";
